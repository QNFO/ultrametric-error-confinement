...
1. **Asymmetric decoder:** The simulation assumed a symmetric decoder, but with $p = 2$ children per node and a tie-breaker fixed to logical $0$, the decoder becomes AND-not-majority, making logical $1$ exponentially fragile. Only logical $0$ was tested.
...
## 5. A Framework for Proof-Aware Physics

Drawing on the taxonomy of Section 1, the case study of Section 2, and the philosophical position of Section 3, I propose a framework for thinking about the relationship between mathematical proof and physical validation.

### 5.1 The Proof-Physics Contract
...
But Lean cannot verify proof types 2–4 without additional machinery — and critically, it cannot verify the physical correspondence of any assumption. A Lean proof that "under i.i.d. noise with rate $p < p_{\text{th}}$, the logical error rate is exponentially suppressed" tells you that the mathematics is internally consistent. It does not tell you that any physical qubit experiences i.i.d. noise, or that $p_{\text{th}}$ is achievable in hardware, or that the tree geometry can be physically instantiated.

This is not a limitation of Lean specifically — it is a limitation of any formal system. The assumptions gap is not a bug in formal verification; it is a feature of the relationship between formal systems and the world.

### 5.3 The Ultrametric Test: What Would Success Look Like?

The ultrametric error confinement architecture provides a natural experiment for this framework. Suppose we:

1. State the assumptions explicitly (symmetric decoder, $p \ge 3$, i.i.d. bit-flip noise with rate $p_{\text{err}}$, tree geometry with branching factor $b$).
...
## 6. Conclusion: The Productive Gap
