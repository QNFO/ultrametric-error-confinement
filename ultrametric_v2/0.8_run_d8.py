"""
0.8_run_d8.py -- Run depth=8 experiment with 2000 trials.
d=8: 3^8 = 6561 leaves, total nodes = 9841, barrier B=256.
Target: Wilson CI upper bound below 1e-4 at p_err=0.40.
"""
...
for bit in [0, 1]:
    key = f"d{depth}_bit{bit}"
    res = all_results[key]
    total_errs = sum(r["errors"] for r in res["results"])
...
print(f"\nAt p_err=0.40, d=8: LER={r40['ler']:.4f}, CI=[{r40['ci_lower']:.4f}, {r40['ci_upper']:.4f}]")
below_1e4 = r40['ci_upper'] < 0.0001
below_1e5 = r40['ci_upper'] < 0.00001
print(f"CI below 1e-4: {'YES' if below_1e4 else 'NO'}")
print(f"CI below 1e-5: {'YES' if below_1e5 else 'NO'}")
