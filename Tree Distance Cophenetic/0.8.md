...
tags: cophenetic distance, ultrametric inequality, triadic rigidity, hierarchical clustering, dendrogram, rooted tree, ontology, Page-Wootters, Wheeler-DeWitt, resolution-dependence, generative mechanism, process ontology, structural realism, foundational ontology, mathematical philosophy, information ontology
...
**Branching as symmetry-breaking.** Before the first cut: all items symmetric (undifferentiated cluster). successive differentiation drives symmetry-breaking: resolves "red vs. blue." Further symmetry-breaking within each cluster: "large vs. small." At leaves: items are fully differentiated — each uniquely identifiable.

**Resolution of tension T1 (Static vs. Dynamic).** The tree is what builds; is how the tree is traversed. Both are equally real; neither is reducible to the other. The static/dynamic "tension" is a complementarity — different standpoints on the same structure, analogous to the Page-Wootters resolution in quantum gravity.

**The tree grows.** The tree at any finite $\varepsilon$ is a truncation of a potentially infinite resolution ladder. The tree extends downward as differentiation continues. Reality is not a static tree but a tree-growing process — an open-ended generative structure.
...
**The arrow of time as resolution increase.** The direction of time is from coarse $\varepsilon$ to fine $\varepsilon$ — from root to leaf. This is not reversible because each cut creates new informational structure; reversing requires un-drawing cuts, destroying the coherence of what has already been differentiated.
...
## 5. Development of This Framework

This framework was developed through iterative refinement across four stages:

**Stage 1: Mathematical Derivation.** The core definitions (rooted tree, height function, cophenetic distance) were formalized. The ultrametric inequality was proven from the LCA property. Triadic rigidity was derived as a necessary consequence. All theorems were verified computationally (0.3.py, 70+ checks passed).

**Stage 2: Cross-Reference.** The framework was compared against existing literature in hierarchical clustering (Sokal & Rohlf, Sneath & Sokal), phylogenetics (Hartigan), and quantum gravity (Page & Wootters, DeWitt). The goal was not to claim equivalence but to identify structural patterns that recur across domains — the same mathematical form appearing in different contexts.

**Stage 3: Computational Verification.** Beyond proving the theorems, the framework was implemented as working code. The 0.3.py script verifies ultrametric inequality and triadic rigidity for all triples. The 0.9.py benchmark tests whether TRI can discriminate hierarchical from non-hierarchical structures (result: 76x separation). The cophenetic package makes the tools available for reuse.

**Stage 4: Iterative Refinement.** The document was progressively refined through reader testing (12 issues identified and resolved), citation verification (5 references confirmed), and structural revision (diagrams added, glossary appended, physics bridges rewritten for clarity). Each iteration improved accessibility without compromising rigor.

### 5.1 Limitations

This framework has acknowledged limitations:

1. **Empirical scope.** The mathematical results are proven. The structural bridges to physics and cognition are plausible but not empirically validated. Triadic rigidity provides a falsifiable test, but that test has not yet been applied to real-world data.

2. **Physics minority.** The Page-Wootters construction used as the primary physics analogy is not the mainstream approach to quantum gravity. Section 3.4 addresses this by mapping the tree framework to three alternative frameworks (Barbour, Rovelli, 't Hooft).

3. **Scope boundary.** Not all reality is tree-structured. The framework's claim is bounded: where hierarchy exists, it has the mathematical signature of triadic rigidity. Non-hierarchical domains exist and are not denied.

4. **Accessibility.** The mathematical core (Section 2) is self-contained. Sections 3-4 reward broader background but do not require it. An accessible introduction (0.11.md) and glossary (Appendix A) are provided.
...
| Cross-reference to existing literature | `[EXTERNAL-SOURCE]` — imported as `src_0.2_*.md` | 16 files |
...
| 🟡 | Address limitations identified in §5.1 | ⚠️ Partially addressed; empirical validation remains |
...
**DOI:** 10.5281/zenodo.20208437
**Date:** 2026-05-15
**License:** [License](https://qnfo.org/LICENSE) — Non-commercial use only. Attribution required.
...
- `0.5.md` — Methodology and self-evaluation
